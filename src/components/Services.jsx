...
<div className="relative">
  <div className="relative flex items-center z-1 h-[39rem] mb-5 p-8 border border-n-1/10 rounded-3xl overflow-hidden lg:p-20 xl:h-[46rem]">
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none md:w-3/5 xl:w-auto">
      <img className="object-cover w-full h-full md:object-right" width={800} height={730} alt="smartest" src={service1} />
    </div>
  ...
  <div className="relative grid gap-5 z-1 lg:grid-cols-2">
    <div className="relative min-h-[39rem] border border-n-1/10 rounded-3xl overflow-hidden">
      <div className="absolute inset-0">
        <img src={service2} className="object-cover w-full h-full" width={630} height={750} alt="robot" />
      </div>
   ...
   <ul className="flex items-center justify-between">
    {brainwaveServicesIcons.map((item, index) => (
      <li key={index} className={` rounded-2xl flex items-center justify-center ${index === 2 ? 'w-[4.5rem] h-[3rem] p-0.25 bg-conic-gradient  md:w[4.5rem] md:h-[4.5rem]' : 'flex w-10 h-10 bg-n-6 md:w-15 md:h-15'}`}>
      ...
      <div className="relative h-[20rem] bg-n-8 rounded-xl overflow-hidden md:h-[25rem]">
        <img src={service3} className="object-cover w-full h-full " width={520} height={400} alt="robot" />
        <VideoChatMessage />
        <VideoBar />
      </div>
    </div>
