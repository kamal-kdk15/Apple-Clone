...
const Section = ({
  ...
  return (
    <div id={id} className={`relative ${customPaddings || `py-10 lg:py-16 xl:py-20 ${crosses ? "lg:py-32 xl:py-40" : ''}`} ${className || ''}`}>
      {children}
      <div className="absolute top-0 hidden w-0.25 h-full bg-stroke-1 pointer-events-none left-5 md:block lg:left-7.5 xl:left-10" />
      <div className="absolute top-0 hidden w-0.25 h-full bg-stroke-1 pointer-events-none right-5 md:block lg:right-7.5 xl:right-10" />
      {crosses && (
        <>
        <div 
